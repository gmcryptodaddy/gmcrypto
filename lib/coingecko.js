// lib/coingecko.js
// Wrapper around CoinGecko API.
// If COINGECKO_API_KEY env var is set, uses Demo API key for higher limits
// (~30 calls/min vs ~5-15 for unauthenticated).
//
// Errors thrown include the upstream status code in the message so callers
// can distinguish 404 (coin doesn't exist) from 429 (rate limited) etc.
...
async function fetchCG(path, params = {}) {
  ...
  if (!res.ok) {
    const err = new Error(`CoinGecko error ${res.status}: ${res.statusText}`)
