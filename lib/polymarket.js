...
const MAX_PROBABILITY = 0.97 // Skip near-settled markets — a flat "100%" reads as broken, not "future"
...
export async function getPolymarketFeed() {
  try {
    ...
    return markets
      ...
      .filter(m => m.probability < MAX_PROBABILITY)
      .filter(m => m.volume >= MIN_VOLUME_24H)
      // This is a crypto site — drop sports entirely, they're off-brand.
      .filter(m => m.category !== 'Sports')
      // Crypto first, then finance/tech/policy, each by volume. Keeps the
      // homepage teaser on-brand instead of leading with aliens/politics.
      .sort((a, b) => {
        const rank = { Crypto: 0, Markets: 1, Tech: 1, Policy: 2 }
...
function frameHeadline(question, isYesLeading) {
  const prefix = isYesLeading ? 'Likely:' : 'Unlikely:'
